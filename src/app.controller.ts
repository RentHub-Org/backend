...
export class AppController {
  ...
  async getJsonDev(@Req() req, @Param('cid') cid: string) {
    ...
    if (response.headers['content-type'] != 'application/json') {
      ...
      throw new HttpException(
        { error: 'The requested CID is not a JSON file.' },
        400,
      );
    }
  ...
  async gateway(@Param('cid') cid: string, @Res() res: Response): Promise<any> {
    ...
    res.setHeader(
      'Content-Type',
      serviceResponse.headers['content-type'] || 'application/octet-stream',
    );
